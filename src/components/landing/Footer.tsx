        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-6 sm:gap-8">
          ...
          <div className="col-span-2 sm:col-span-2 lg:col-span-2">
            ...
            <div className="space-y-1.5 sm:space-y-2">
              <a href={`mailto:${contactSettings.email}`} className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground hover:text-foreground transition-colors">
                <Mail className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" /><span className="truncate">{contactSettings.email}</span>
              </a>
              <a href={`tel:${contactSettings.phone.replace(/\s/g, "")}`} className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground hover:text-foreground transition-colors">
                <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" />{contactSettings.phone}
              </a>
              <p className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" />{contactSettings.address}
              </p>
            </div>
          </div>

          <div>
            <h3 className="font-semibold text-foreground text-sm sm:text-base mb-3 sm:mb-4">Platforma</h3>
            <ul className="space-y-2 sm:space-y-2.5">{renderLinks(platformLinks)}</ul>
          </div>

          <div>
            <h3 className="font-semibold text-foreground text-sm sm:text-base mb-3 sm:mb-4">Yordam</h3>
            <ul className="space-y-2 sm:space-y-2.5">{renderLinks(supportLinks)}</ul>
          </div>

          <div className="col-span-2 sm:col-span-1">
            <h3 className="font-semibold text-foreground text-sm sm:text-base mb-3 sm:mb-4">Huquqiy</h3>
            <ul className="space-y-2 sm:space-y-2.5">{renderLinks(legalLinks)}</ul>
          </div>
        </div>

        {/* Bottom */}
        <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-border flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
          <p className="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
            © {new Date().getFullYear()} ShohruxDigital.uz. Barcha huquqlar himoyalangan.
          </p>
          <div className="flex items-center gap-3">
            <a href={contactSettings.telegram_url} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors text-muted-foreground hover:text-foreground" aria-label="Telegram">
            ...
            <a href={contactSettings.instagram_url} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors text-muted-foreground hover:text-foreground" aria-label="Instagram">
              <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="currentColor">
